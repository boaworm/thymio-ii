...
async def phase2_escape(node, client):
    ...
    tilt_ticks = 0
    blocked_ticks = 0
    had_left_wall = False   # tracks whether we've been following a wall
    ...
    while True:
        ...
        if front_blocked or left_blocked:
            blocked_ticks += 1
            had_left_wall = True  # wall ahead counts as "had wall"
            ...
            if blocked_ticks <= RETREAT_TICKS:
                ...
                if not rear_blocked:
                    ...
                    state = "STUCK→arc_right"
            else:
                # Phase 2: arc right — wall is on left, so left=fast right=slow.
                left_motor  = TURN_OUTER
                right_motor = TURN_INNER
                state = "BLOCKED→right"

        elif not has_left_wall and had_left_wall:
            # Had the wall, just lost it — arc left to hug around corner.
            # Wall side (left) goes slow, far side (right) goes fast.
...
async def connect(kwargs):
    """Connect to the Thymio, retrying on failure."""
    while True:
    ...
    try:
        while True:
            try:
                ...
                client, node = await connect(kwargs)
                # Skip startup on reconnect, jump straight to escape.
                startup_done = True
